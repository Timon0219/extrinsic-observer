...
def check_dissolve_subnet(events):
    point = False
    for event in events:
...
def report_dissolve_subnet(current_block_number, time_stamp):
    ...
    embed = {
        "title": "🌟 __ NEW executing swap clodkey __ 🌟",
        "description": "",
...
def dissolve_subnet(substrate, block_number):
    ...
    has_dissolve_subnet_idx= check_dissolve_subnet(events)
    print (has_dissolve_subnet_idx)
    if has_dissolve_subnet_idx == True:
